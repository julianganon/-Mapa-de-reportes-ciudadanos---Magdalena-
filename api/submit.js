...
export default async function handler(req, res) {
  ...
  try {
    ...
    // FIX: filtramos solo por IP (sin índice compuesto) y aplicamos
    // el filtro de fecha en memoria para evitar el error de Firestore.
    if (clientIp !== 'IP_DESCONOCIDA') {
      const inicioHoy = new Date();
      inicioHoy.setHours(0, 0, 0, 0);
      const inicioHoyISO = inicioHoy.toISOString();

      const snapIp = await db.collection('reports')
        .where('ip', '==', clientIp)
        .get();

      const reportesHoy = snapIp.docs.filter(doc => {
        const ts = doc.data().timestamp;
        return ts && ts >= inicioHoyISO;
      });

      if (reportesHoy.length >= IP_LIMITE_DIARIO) {
        await registrarSospechoso({
          evento: 'RATE_LIMIT',
          detalle: `Límite diario por IP alcanzado: ${reportesHoy.length} reportes hoy`,
          ip: clientIp, uid, email: userEmail, userAgent, userName,
          mensaje: typeof description === 'string' ? description : '',
          cats: Array.isArray(cats) ? cats : [],
          lat: typeof lat === 'number' ? lat : null,
        ...
        console.log(`[LIMITE_DIARIO_IP] IP: ${clientIp} | Reportes hoy: ${reportesHoy.length}`);
