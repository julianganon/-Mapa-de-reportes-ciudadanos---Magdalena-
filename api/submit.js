...
// ── Límite diario de reportes exitosos por IP ────────────────────
// 10 es suficiente para cualquier vecino o familia legítima,
// y corta cualquier ataque masivo antes de que cause daño.
const IP_LIMITE_DIARIO = 10;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 3b. Límite diario de reportes por IP ─────────────────────
    // Cuenta reportes exitosos de esta IP en el día de hoy (hora AR).
    // Limita a 10 por día — suficiente para cualquier vecino o familia,
    // imposible de sortear rotando cuentas desde la misma red.
    if (clientIp !== 'IP_DESCONOCIDA') {
      const inicioHoy = new Date();
      inicioHoy.setHours(0, 0, 0, 0);

      const snapDiario = await db.collection('reports')
        .where('ip', '==', clientIp)
        .where('timestamp', '>=', inicioHoy.toISOString())
        .get();

      if (snapDiario.size >= IP_LIMITE_DIARIO) {
        await registrarSospechoso({
          evento: 'RATE_LIMIT',
          detalle: `Límite diario por IP alcanzado: ${snapDiario.size} reportes hoy`,
          ip: clientIp, uid, email: userEmail, userAgent, userName,
          mensaje: typeof description === 'string' ? description : '',
          cats: Array.isArray(cats) ? cats : [],
          lat: typeof lat === 'number' ? lat : null,
        ...
        console.log(`[LIMITE_DIARIO_IP] IP: ${clientIp} | Reportes hoy: ${snapDiario.size}`);
        return res.status(429).json({ error: 'Límite de reportes diarios alcanzado para esta red' });
      }
    }
    ...
    const report = {
      ...
      ip: clientIp,   // necesario para el límite diario por IP
      fingerprint: fingerprint || null,
    };
