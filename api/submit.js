...
// ── Caché local de UIDs bloqueados (evita lecturas a Firestore) ──
// Si un UID ya fue detectado como bloqueado en esta instancia,
// no se vuelve a consultar Firestore — ahorra lecturas ante ataques.
const LOCAL_BLOCKED_CACHE = new Set();

// ── Verificar si UID está bloqueado ─────────────────────────────
async function isBlocked(uid) {
  // 1. Caché en memoria — cero lecturas a Firestore
  ...
  // 3. Firestore — solo si no estaba en caché
  try {
    const doc = await db.collection('blocked_uids').doc(uid).get();
    if (doc.exists && doc.data().desbloqueado === false) {
      LOCAL_BLOCKED_CACHE.add(uid); // cachear para próximas requests
      return true;
    }
  } catch (e) {
...
export default async function handler(req, res) {
  ...
  // ── Verificar IP bloqueada (antes del token — ahorra procesamiento) ──
  const ipBloqueada = await isBlockedIP(clientIp);
  if (ipBloqueada) {
    console.log(`[IP_BLOQUEADA] IP: ${clientIp}`);
    // Respuesta falsa — no revelar que está bloqueado
  ...
  try {
    ...
    // Detecta variantes como "s p a m", "spàm", "SPAM" etc.
